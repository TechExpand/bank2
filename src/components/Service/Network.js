...
const mainurl = "https://bank126.herokuapp.com/api"
...
export const getBlogPost = (setData) =>{
  let status;
  let id = localStorage.getItem('id');
  fetch(`${mainurl}/blog/`, {
method: 'get',
...
export const getProfileList = (setList, setqueryList) =>{
  let status;
  let id = localStorage.getItem('id');
...
export const getTransaction = (setList,handleTotalTransaction) =>{
  let status;
  let id = localStorage.getItem('id');

  fetch(`${mainurl}/transaction/${id}`, {
method: 'get',
headers: {
  'Content-Type':  'application/json',
},
...
let total = 0
    ...
    setList(res)
     res.forEach(e => {
      total = total + Number(e.amount.toString())
      handleTotalTransaction(total)
    });
  } 
  });
} 
